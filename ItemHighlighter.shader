Shader "witnnShaders/ItemHighlighter"
{
	Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                norm = normalize(norm);
                float3 offset = norm * _OtlWidth;

                float4 pos = mul(UNITY_MATRIX_MV, v.vertex);
                pos.xy += offset.xy;

                o.pos = mul(UNITY_MATRIX_P, pos);
                o.worldPos = mul(UNITY_MATRIX_M, v.vertex).xyz;  // Store world position
                return o;
            }
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float fade = smoothstep(_OtlDist, _OtlDist + 1.0, distance);

                return _OtlColor * (1.0 - fade); // Fade outline based on distance
            }
            ENDCG
        }
